...
public class StatisticsForm extends JPanel {
    ...
    private void initComponent() {
        ...
        openRevenue = new Button("Thông số", 80, 30, Color.decode("#333333"));
    ...
    public void setDataRevenueTable() {
        ...
        productTable.myAddRow("Tổng doanh thu: ", ""+BillBUS.getRenevue(start, end));
        productTable.myAddRow("Tổng số đơn hàng: ", ""+BillBUS.getTotalBill(start,end));
        productTable.myAddRow("Số tiền trung bình của 1 đơn hàng: ", ProjectUtil.toMoney(BillBUS.getAVG(start,end))); 
        productTable.myAddRow("Tổng số sản phẩm bán được: ", 
                String.valueOf(BillDetailBUS.getProductSales(start,end) + BillDetailBUS.getComboSale(start,end)));
    }
