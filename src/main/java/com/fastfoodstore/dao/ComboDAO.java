...
public class ComboDAO implements DAOInterface<ComboDTO> {
    ...
    public int insert(ComboDTO t) {
        ...
        try {
            ...
            String sql = "INSERT INTO `combo` (`comboCode`, `comboName`, `comboPrice`, `numberOfProduct`, `comboImage`, `groupCode`, `inMenu`, `inSys`)"
                    + " VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    public int update(ComboDTO t) {
        ...
        try {
            ...
            String sql = "UPDATE `combo`"
                    + " SET `comboCode` = ?, `comboName` = ?, `comboPrice` = ?, `numberOfProduct` = ?, `comboImage` = ?, `groupCode` = ?, `inMenu` = ?, `inSys` = ?"
            ...
            pst.setBoolean(7, t.getInMenu());
            pst.setBoolean(8, t.isInSys());
            pst.setString(9, t.getComboCode());
    ...
    public ArrayList<ComboDTO> selectAll() {
        ...
        try {
            Connection connection = ConnectionData.getConnection();
            String sql = "SELECT * FROM combo where inSys = 1";
            PreparedStatement pst = connection.prepareStatement(sql);
    ...
    public ComboDTO selectById(String id) {
        ...
        try {
            Connection connection = ConnectionData.getConnection();
            String sql = "SELECT * FROM combo WHERE comboCode = ? and inSys = 1";
            PreparedStatement pst = connection.prepareStatement(sql);
            pst.setString(1, id);
            ResultSet rs = pst.executeQuery();
    ...
    public ArrayList<ComboDTO> selectByCondition(String condition, String colName) {
        ...
        try {
            Connection connection = ConnectionData.getConnection();
            String sql = "SELECT * FROM combo where " + condition + " and inSys = 1";
            PreparedStatement pst = connection.prepareStatement(sql);
